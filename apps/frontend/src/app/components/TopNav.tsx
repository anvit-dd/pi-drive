            ...
            <div className='hidden md:flex flex-1 min-w-0'>
              <form className='w-full'>
                <TextField.Root
                  ...
                  size='2'
                  radius='full'
                  className='bg-transparent pr-3 py-2 shadow-sm !font-sans w-full'
                  value={searchInput}
                  onChange={(e) => {
          ...
          {!isSharedView && (
            <DropdownMenu.Root>
              ...
              <DropdownMenu.Content
                ...
                sideOffset={8}>
                <div className='px-3 py-2 mb-2 hover:bg-[var(--gray-a3)] rounded cursor-default outline-none'>
                ...
                <div className='flex items-center justify-between rounded-lg gap-x-0.5 border'>
                  {themes.map(({ value, icon: Icon }) => (
                    <button
                      key={value}
                      onClick={() => setTheme(value)}
                      className={cn(
                        "p-1.5 rounded-md w-full flex items-center justify-center transition-colors duration-200",
                        theme === value
                          ? "bg-[var(--accent-a3)] text-[var(--accent-a11)]"
                          : "hover:bg-[var(--accent-a3)] hover:text-[var(--accent-a11)]"
                      )}>
                      <Icon size={18} />
                    </button>
                  ))}
                ...
                <DropdownMenu.Item
                  className='px-3 py-2 mt-2 hover:bg-[var(--accent-a3)] rounded cursor-pointer flex items-center gap-x-2 outline-none'
