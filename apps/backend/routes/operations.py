    ...
    try:
        ...
        for item_path in items_list:
            ...
            if not await asyncio.to_thread(src_full_path.exists):
                raise HTTPException(status_code=404, detail=f"Source path not found: {item_path}")
            ...
            if src_resolved == dest_resolved or str(dest_resolved).startswith(str(src_resolved) + os.sep):
                raise HTTPException(
                    status_code=400,
        ...
        return JSONResponse(
            content={"message": "Moved contents successfully."},
            status_code=200
        )
    ...
    try:
        ...
        for item_path in items_list:
            ...
            if await asyncio.to_thread(src_full_path.is_dir):
                await asyncio.to_thread(shutil.copytree, str(src_full_path), str(new_location))
            else:
                await asyncio.to_thread(shutil.copy2, str(src_full_path), str(new_location))

        return JSONResponse(
            content={"message": "Copied contents successfully."},
            status_code=200
        )
